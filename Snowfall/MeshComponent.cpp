...
void MeshRenderingSystem::InitializeSystem(EntityManager *eManager, ComponentManager *cManager, EventManager *evManager, PrototypeManager *pManager)
{
	m_eManager = eManager;
	m_cManager = cManager;
}
...
{
	for (MeshRenderComponent *render : m_cManager->GetComponents<MeshRenderComponent>())
	{
		TransformComponent *transform = render->Owner.GetComponent<TransformComponent>();
		if (!render->Batch)
			BatchNewObject(*render, *transform);
	}
	for (MeshRenderComponent *render : m_cManager->GetDeadComponents<MeshRenderComponent>())
	{
		if (render->Batch) 
...
void MeshRenderingSystem::BatchDynamic(BatchState& state, MeshRenderComponent& mcomp, TransformComponent& tcomp, bool instanced)
{
	...
	{
		handle = Snowfall::GetGameInstance().GetMeshManager().CreateGeometry(mesh.Vertices.size(), mesh.Indices.size());

		Snowfall::GetGameInstance().GetMeshManager().WriteGeometryVertices(handle, mesh.Vertices.data(), 0, static_cast<int>(mesh.Vertices.size()));
		Snowfall::GetGameInstance().GetMeshManager().WriteGeometryIndices(handle, mesh.Indices.data(), 0, static_cast<int>(mesh.Indices.size()));

		state.StateChange.InstanceCount = 1;
		state.Mesh = masset;
		state.Geometry = handle;
	}
	state.Handles.push_back(BatchMember());
